...
- Store evaluation prompts in [evaluation/queries.json](evaluation/queries.json) and run `python evaluation/generate_responses.py` to call `get_weather` directly, producing reproducible responses in [evaluation/responses.json](evaluation/responses.json).
- Score those replies with `python evaluation/score.py --responses evaluation/responses.json` to report `ToolCallSuccessRate`, `SchemaCompleteness`, and `MockDataDiversity` so agents can verify weather schema stability.
- Keep the evaluation directory committed so agents can work entirely within GitHub and avoid dropping dataset artifacts on local disks.
